...
def draw_line(tank1_coord, tank2_coord, shot):
    if int(tank1_coord) and int(tank2_coord) and int(shot) < 800 and int(tank1_coord) and int(tank2_coord) and int(shot) > 0:
        myLine = list('-'*800)
...
def play_game():
    draw_tanks()
    tank1, tank2 = input('Please give the coordinates of the two tanks (two number along x-axis with space between them no comma)').split()
    while True:
